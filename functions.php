...
function ttrust_scripts() {	
	...
	wp_register_script('vendor', get_bloginfo('template_url').'/dist/js/vendor.js', array('jquery'), '1.0', true);	
	wp_register_script('blog', get_bloginfo('template_url').'/dist/js/app.js', null, '1.0', true);	
	wp_register_script('jqueryslider', get_bloginfo('template_url').'/js/jquery.slider.min.js', null, '1.0', true);	

  wp_localize_script( 'blog', 'AjaxData', array( 'url' => admin_url( 'admin-ajax.php' )));   
...
function ajax_handler() {
  switch ($_REQUEST['fn']) {
    case 'instabinge':
      $output = get_instagram_feed($_REQUEST['count'], $_REQUEST['nextUrl']);
      break;
  }
...
// instagram ajax
// ------------------------
function get_instagram_feed($count = 20, $next_url) {
  //$access_token = get_option( 'instagram_access_token', 'nothing' );
  //$url = 'https://api.instagram.com/v1/users/self/feed?access_token=' . $access_token . '&count=' . $count;
  $url = 'https://api.instagram.com/v1/users/1420408305/media/recent?client_id=5f64f3eae8ae4b2487c795ebdbcd3795&count=15&access_token=1420408305.5f64f3e.d0d3435e398344a9a74755ecfc1ed12d';
